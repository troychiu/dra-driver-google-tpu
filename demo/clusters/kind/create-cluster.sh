...
# This script builds a kind image and creates a kind cluster
# with CDI support and the driver image loaded.
#
...
if [ "${BUILD_KIND_IMAGE}" = "true" ]; then
	${CURRENT_DIR}/scripts/build-kind-image.sh
fi
${CURRENT_DIR}/scripts/create-kind-cluster.sh
...
if [ "${EXISTING_IMAGE_ID}" != "" ]; then
	${CURRENT_DIR}/scripts/load-driver-image-into-kind.sh
fi
