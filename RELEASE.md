...
The pushed tag automatically triggers a Cloud Build job (defined in [cloudbuild.yaml](cloudbuild.yaml)). This job executes `make release`, which packages, builds, and pushes the artifacts to the staging registry:
...
2.  **Retrieve Digests**: Use `crane` to get the immutable SHA256 digests for the staged artifacts:
    ...
    ```
3.  **Open Promotion PR**: Open a Pull Request against the [kubernetes/k8s.io](https://github.com/kubernetes/k8s.io) repository. Add these retrieved digests to the appropriate images file (e.g., `registry.k8s.io/images/k8s-staging-images/images.yaml`) to sync them to the production `registry.k8s.io`.
...
1.  **Generate Release Notes**: Run the `release-notes` tool to compile all PRs merged since the last release:
    ```bash
    release-notes \
    --github-org kubernetes-sigs \
    --github-repo dra-driver-google-tpu \
    --branch main \
    --start-rev v0.0.0 \
    --end-rev v0.1.0 \
